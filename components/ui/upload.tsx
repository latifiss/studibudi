    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 504) {
          throw new Error(
            'The request timed out. Please try again with a shorter document.'
          )
        }
      ...
      if (data.success && data.quiz) {
        ...
        const savedQuizData = savedQuizResponse.ok
          ? await savedQuizResponse.json()
          : null
        ...
        localStorage.setItem(
          'currentQuizId',
          savedQuizData?.quiz?.id ?? ''
        )

        onQuizGenerated?.(currentQuiz)

        const url = savedQuizData?.quiz?.id
          ? `/quiz?historyId=${savedQuizData.quiz.id}`
          : '/quiz'
    ...
    <>
      ...
      <div
        className={cn(
          ...
          'relative',
          isAlternate
            ? 'w-full max-w-[961px] h-[349px]'
            : 'w-full max-w-[322px] sm:max-w-[500px] lg:max-w-[400px] h-[231px] sm:h-[279px] lg:h-[612px]',
        ...
        >
          <div
            className={cn(
              ...
              isAlternate
                ? 'max-w-[320px]'
                : 'max-w-[189.98px] sm:max-w-[320px] lg:max-w-[320px]'
            )}
          >
            <div
              className={cn(
                ...
                isAlternate
                  ...
                  : 'w-[60px] h-[60px] sm:w-[100px] sm:h-[100px] lg:w-[140px] lg:h-[140px]'
              )}
            >
              <DocsIcon
              ...
              <p
                className={cn(
                  ...
                  isAlternate
                    ...
                    : 'text-[8px] sm:text-[12px] lg:text-[14px]',
            ...
            >
              <button
                ...
                className={cn(
                  ...
                  'w-full',
                  isAlternate
                    ? 'h-[40px] sm:h-[44px] lg:h-[48px] text-[14px] sm:text-[16px] lg:text-[18px]'
                    : 'h-[24px] sm:h-[40px] lg:h-[48px] text-[9px] sm:text-[14px] lg:text-[18px]'
                )}
              >
                Choose file
              </button>
