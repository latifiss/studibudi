...
const ProfileModal = ({ isOpen, onClose, tier, onLogout, onUpgrade, onCancel, userEmail, userName, userImage, anchorRef }: ProfileModalProps) => {
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (window.innerWidth - rect.right < 12) setPosition({ top: rect.bottom + 8, right: 12 })
      void modalWidth
    }
    updatePosition()
  ...
  const handleCancel = async () => {
    ...
    try { await onCancel() } catch (error) { console.error('Unable to open cancellation flow:', error); setIsCancelling(false) }
  }
    ...
    <div ref={modalRef} className="fixed z-[9999] w-70" style={{ top: position.top, right: position.right }}>
      <div className={cn('bg-white dark:bg-[#1a1a2e] rounded-2xl w-full', 'shadow-[0_8px_16px_-12px_rgba(0,0,0,0.8),0_12px_16px_-12px_rgba(79,76,240,0.64)]', 'relative overflow-hidden', 'border border-[#E5E5E5] dark:border-[#2a2a3e]')}>
        ...
        <div className="py-2">
          ...
          {liveTier === 'pro' && onCancel && <button onClick={handleCancel} disabled={isCancelling} className={cn('flex items-center w-full px-5 h-13.5', 'hover:bg-gray-50 dark:hover:bg-white/5', 'transition-colors duration-200', 'font-text text-[14px] font-medium text-[#FE1212] disabled:opacity-50')}>{isCancelling ? 'Opening...' : 'Cancel subscription'}</button>}
