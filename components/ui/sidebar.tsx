...
interface ChatHistoryItem { id: string; title: string; href: string; active?: boolean }
interface SidebarProps { className?: string; onNewQuiz?: () => void }
...
const Sidebar = ({ className, onNewQuiz, ...props }: SidebarProps) => {
  ...
  useEffect(() => {
    if (!authenticated) { setQuizHistory([]); return }
    const loadQuizHistory = async () => {
      try {
        const response = await fetch('/api/quizzes', { cache: 'no-store' })
        if (!response.ok) { setQuizHistory([]); return }
        const data = await response.json()
        setQuizHistory((data.quizzes ?? []).map((item: { id: string; title?: string }) => ({
          id: item.id, title: item.title || 'Untitled quiz', href: `/quiz?historyId=${item.id}`,
        })))
      } catch (error) { console.error('Failed to load quiz history:', error); setQuizHistory([]) }
    }
    loadQuizHistory()
  }, [authenticated])
  ...
  return (
    <aside className={cn(
      'fixed inset-y-0 left-0 flex flex-col h-dvh min-h-0 py-0 w-full lg:w-70',
      'bg-background border-r border-border transition-colors duration-200 overflow-hidden',
      className
    )} {...props}>
      ...
      <div className="flex flex-col min-h-0 flex-1 px-4 sm:px-5">
        <div className="shrink-0 w-full">
          <ChatButton onClick={handleNewQuiz} className="w-full" />
          <div className="h-5" />
        </div>
      ...
      <div className="shrink-0 w-full"><SidebarProfile /></div>
    </aside>
  )
}
