        ...
        <div className="hidden sm:flex items-center gap-6">
          ...
          {authenticated ? (
            <button ref={profileRef} onClick={handleProfileClick} className="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center hover:opacity-70 transition-opacity focus:outline-none" aria-label="Open profile" aria-expanded={isProfileOpen}>
              {user?.image ? <Image width={34} height={34} src={user.image} alt={displayName} className="w-full h-full object-cover" /> : <div className="w-full h-full rounded-full bg-[#F5F5F5] flex items-center justify-center"><ProfileIcon className="w-5 h-5 text-[#333333]" /></div>}
            </button>
          ) : (
        ...
        <div className="flex sm:hidden items-center gap-4">
          ...
          {authenticated ? (
            <button ref={profileRef} onClick={handleProfileClick} className="w-8 h-8 rounded-full overflow-hidden flex items-center justify-center hover:opacity-70 transition-opacity focus:outline-none" aria-label="Open profile" aria-expanded={isProfileOpen}>
              {user?.image ? <img src={user.image} alt={displayName} className="w-full h-full object-cover" /> : <div className="w-full h-full rounded-full bg-[#F5F5F5] flex items-center justify-center"><ProfileIcon className="w-4 h-4 text-[#333333]" /></div>}
            </button>
