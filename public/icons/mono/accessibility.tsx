import * as React from "react"
import { SVGProps } from "react"

interface AccessibilityIconProps extends SVGProps<SVGSVGElement> {
  size?: number | string
  color?: string
}

const AccessibilityIcon = ({
  size = 24,
  color = "#333",
  style,
  ...props
}: AccessibilityIconProps) => {
  const pixelSize = typeof size === 'number' ? size : parseInt(size, 10) || 24
  
  const scale = pixelSize / 24
  const aspectRatio = 1 
  
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={pixelSize}
      height={pixelSize * aspectRatio}
      fill="none"
      viewBox="0 0 24 24"
      style={{
        flexShrink: 0,
        ...style,
      }}
      {...props}
    >
      <path
        fill={color}
        d="M14.48 3.46a2.47 2.47 0 0 0-3.405-2.271A2.45 2.45 0 0 0 9.79 4.49a2.45 2.45 0 0 0 4.69-1.03Zm1.74 18-.72-3.37v-.03l-.61-3.4-.5-3.43a1.6 1.6 0 0 1 .32-1.08l.02-.02c.22-.29.54-.48.9-.55h.01c.82-.14 1.62-.34 2.4-.6.76-.27 1.5-.61 2.21-1.01a.95.95 0 0 0 .19-1.12.9.9 0 0 0-.44-.41 1 1 0 0 0-.63-.06c-2.63.8-4.93 1.25-7.27 1.27-2.35.01-4.71-.4-7.45-1.3a.94.94 0 0 0-1.04.5A1 1 0 0 0 3.79 8c.7.38 1.45.72 2.2.99.79.25 1.59.45 2.4.58.37.08.7.29.92.59.23.31.34.7.31 1.09l-.5 3.4v.04c-.4 2.27-.79 4.56-1.3 6.8a1 1 0 0 0 1.68.92 1 1 0 0 0 .3-.63l1.62-6.55v-.03a.62.62 0 0 1 .6-.44h.01a.62.62 0 0 1 .6.48l1.61 6.54.01.04a1 1 0 0 0 1.55.7 1 1 0 0 0 .42-1.06Z"
      />
    </svg>
  )
}

export default AccessibilityIcon