...
/**
 * Extract text from a DOCX directly from the uploaded bytes.
 *
 * Mammoth's Node API expects a Buffer, path, or other supported file option.
 * Passing an ArrayBuffer here causes the "Could not find file in options"
 * error, so use the Node Buffer form explicitly.
 */
...
async function extractTextFromPPTX(bytes: Uint8Array): Promise<string> {
  try {
    ...
    const slideNames = zip
      ...
      .sort(
        (a, b) =>
          Number(a.match(/slide(\d+)\.xml/i)?.[1] || 0) -
